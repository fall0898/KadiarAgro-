      ...
      @if (kpis) {
        <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
          ...
          <div class="bg-white rounded-xl border border-neutral-200 p-4 hover:shadow-md transition-shadow">
            ...
            <p class="text-xs text-neutral-500 font-medium">Total Ventes</p>
          ...
          <div class="rounded-xl border p-4 hover:shadow-md transition-shadow"
            ...
            <div class="flex items-start justify-between mb-3">
              <div class="w-9 h-9 rounded-lg flex items-center justify-center" [class]="kpis.solde_net_fcfa >= 0 ? 'bg-primary-100' : 'bg-red-100'">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
        ...
        <div class="xl:col-span-2 bg-white rounded-xl border border-neutral-200 p-5">
          <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-neutral-800">Ventes vs Dépenses par exploitation</h3>
            <a routerLink="/app/finance" class="text-xs text-primary-600 hover:underline font-medium">Voir le détail →</a>
          </div>
          @if (barChartData.labels && barChartData.labels.length > 0) {
            ...
            <div class="h-56 flex items-center justify-center text-sm text-neutral-400">Chargement du graphique...</div>
          }
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-neutral-200">
          ...
          <div class="divide-y divide-neutral-50">
            ...
            @empty {
              <p class="px-5 py-10 text-center text-sm text-neutral-400">Aucune dépense récente</p>
            }
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-neutral-200">
          <div class="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
            <h3 class="font-semibold text-neutral-800">Dernières ventes</h3>
          ...
          <div class="divide-y divide-neutral-50">
            @for (v of ventesRecentes; track v.id) {
              <div class="px-5 py-3 flex items-center justify-between hover:bg-neutral-50 transition-colors">
                <div class="min-w-0">
                  <p class="text-sm font-medium text-neutral-700 truncate">{{ v.produit }}</p>
                  <p class="text-xs text-neutral-400 mt-0.5">{{ v.date_vente | dateFr }} · {{ v.acheteur || 'Acheteur inconnu' }}</p>
                </div>
                <span class="text-sm font-semibold text-primary-600 ml-3 flex-shrink-0">+{{ v.montant_total_fcfa | currencyFcfa }}</span>
              </div>
            }
            @empty {
              <p class="px-5 py-10 text-center text-sm text-neutral-400">Aucune vente récente</p>
            }
          </div>
        </div>
    ...
    this.api.get<FinanceParChamp[]>('finance/par-champ').subscribe(data => {
      this.barChartData = {
        ...
        datasets: [
          {
            label: 'Ventes',
            data: data.map(d => d.total_ventes_fcfa),
          ...
          {
            label: 'Dépenses',
            data: data.map(d => d.total_depenses_fcfa),
